...
const CourseViewer: React.FC<CourseViewerProps> = ({ 
  ...
  const progressPercentage = useMemo(() => {
    ...
    localCourse.modules.forEach(m => {
        m.lessons.forEach(l => {
            total++;
            // A lesson is complete if it has content AND a custom image
            if (l.content && l.imageUrl) completed++;
        });
    });
  ...
  useEffect(() => {
    const fetchLessonData = async () => {
      ...
      // Find current state of the lesson in the course object
      ...
      if (!needsContent && !needsImage) {
        // Sync active lesson with local course state if needed
      ...
      try {
        ...
        // Always generate image if it's missing
  ...
  const handleBulkGenerate = async () => {
    ...
    for (const item of lessonsToGenerate) {
        ...
        try {
            // Check if we already have the content locally to avoid re-fetching if only image is missing
            ...
            // Critical: Update State immediately after this lesson is done
            setLocalCourse(prev => {
                const newModules = [...prev.modules];
                // Clone the specific module
            ...
            // If user is currently viewing this lesson, update view
      ...
      <html>
        <head>
          ...
          <style>
            ...
            body { font-family: 'Inter', sans-serif; color: #000; line-height: 1.6; margin: 0; padding: 0; background: white; -webkit-print-color-adjust: exact; }
            .page-container { width: 100%; max-width: 210mm; margin: 0 auto; padding: 20mm; box-sizing: border-box; overflow-wrap: break-word; }
            .cover-page { height: 100vh; width: 100%; display: flex; flex-direction: column; justify-content: center; text-align: center; page-break-after: always; padding: 20mm; box-sizing: border-box; }
            ...
            @media print { body { margin: 0; } .cover-page { height: 100vh; margin: 0; } .page-container { width: 100%; max-width: none; padding: 15mm; } a { text-decoration: none; color: #000; } }
          </style>
        </head>
        <body>
          ...
          <div class="page-container">
            ...
            ${localCourse.modules.map((module, mIdx) => `
                ...
                `).join('')}
            `).join('')}
            <div class="footer"><p>Generato con CourseFlix AI - Piattaforma di E-Learning Intelligente</p></div>
          </div>
  ...
  const handleDownloadLandingPage = () => {
     ...
     <head>
         ...
         <style>
             ...
             .hero { background-image: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('${localCourse.thumbnailUrl}'); background-size: cover; background-position: center; color: white; padding: 100px 20px; text-align: center; }
             ...
             .btn-cta { background: #cc0000; color: white; padding: 20px 40px; font-size: 24px; font-weight: bold; border: none; border-radius: 8px; cursor: pointer; text-decoration: none; display: inline-block; transition: transform 0.2s; margin-top: 20px; box-shadow: 0 10px 20px rgba(204,0,0,0.3); }
     ...
     <body>
         ...
         <div class="container">
             <div class="section">
                 ...
                 <div style="font-size: 18px; line-height: 1.6;">${courseIntroHtml || `<p>${localCourse.description}</p>`}</div>
             </div>
             <div class="section">
                 <h2>Programma del Corso</h2>
                 <ul class="module-list">
                     ${localCourse.modules.map((m, i) => `<li class="module-item"><span class="module-title">Modulo ${i+1}: ${m.title}</span><span class="module-count">${m.lessons.length} Lezioni</span></li>`).join('')}
                 </ul>
             </div>
  ...
  return (
    <div className="min-h-screen bg-[#141414] text-white font-sans flex flex-col">
      <input type="file" ref={fileInputRef} onChange={handleImportCourse} className="hidden" accept=".json" />
      ...
      <nav className="fixed top-0 w-full z-50 bg-black/90 backdrop-blur-md border-b border-white/10 px-6 py-4 flex justify-between items-center shadow-lg h-[72px]">
        <div className="flex items-center gap-4">
             <div className="text-red-600 font-extrabold text-2xl tracking-tighter cursor-pointer hover:scale-105 transition" onClick={onExit}>COURSEFLIX</div>
            {isBulkGenerating && (
                <div className="hidden md:flex items-center gap-3 bg-neutral-800 px-4 py-2 rounded-full border border-neutral-700 shadow-red-900/20 shadow-lg">
                    ...
                    <span className="text-sm font-medium text-gray-200">Creazione corso... <span className="text-red-500">{progressPercentage}%</span></span>
                </div>
            )}
        </div>
       
        <div className="flex gap-4 items-center">
            <button onClick={handleDownloadLandingPage} className="hidden md:flex items-center gap-2 text-white bg-green-600 hover:bg-green-700 px-4 py-2 rounded-lg text-sm font-bold uppercase transition shadow-lg hover:scale-105" title="Scarica la Landing Page HTML"><LayoutTemplate className="w-5 h-5" /> Landing Page (HTML)</button>
            <button onClick={handleDownloadFullCoursePDF} className="hidden md:flex items-center gap-2 text-white bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg text-sm font-bold uppercase transition shadow-lg hover:scale-105" title="Scarica E-Book PDF"><BookOpen className="w-5 h-5" /> Scarica E-book (PDF)</button>
            <div className="flex bg-neutral-800 rounded-lg p-1 border border-neutral-700">
                <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 text-gray-300 hover:text-white px-3 py-2 text-sm font-semibold uppercase transition hover:bg-neutral-700 rounded"><Upload className="w-5 h-5" /></button>
                <div className="w-[1px] bg-neutral-700 mx-1"></div>
                <button onClick={handleExportCourse} className="flex items-center gap-2 text-gray-300 hover:text-white px-3 py-2 text-sm font-semibold uppercase transition hover:bg-neutral-700 rounded"><Download className="w-5 h-5" /></button>
            </div>
            <button onClick={handleBulkGenerate} disabled={isBulkGenerating || progressPercentage === 100} className={`hidden md:flex items-center gap-2 bg-gradient-to-r from-red-700 to-red-900 hover:from-red-600 hover:to-red-800 px-5 py-2.5 rounded text-sm font-bold uppercase tracking-wider transition shadow-lg ${isBulkGenerating || progressPercentage === 100 ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105'}`}>
                {progressPercentage === 100 ? <><CheckCircle className="w-5 h-5" /> Completato</> : <><Sparkles className="w-5 h-5" /> {isBulkGenerating ? "Generazione..." : "Genera Corso Completo"}</>}
            </button>
            <div className="w-10 h-10 bg-red-600 rounded flex items-center justify-center font-bold text-lg shadow-lg">{localCourse.title.charAt(0)}</div>
        </div>
      </nav>
      ...
      <div className="flex flex-col lg:flex-row flex-grow mt-[72px] h-[calc(100vh-72px)]">
        ...
        <div className="lg:w-2/3 bg-black overflow-y-auto scrollbar-hide">
            {activeLesson === null ? (
                <div className="relative w-full min-h-full">
                     <div className="absolute inset-0 bg-cover bg-center opacity-40" style={{ backgroundImage: `url(${localCourse.thumbnailUrl})` }} />
                     ...
                     <div className="relative z-10 px-8 md:px-16 py-12 flex flex-col justify-end min-h-[60vh]">
                         ...
                         <div className="flex gap-4 mb-12">
                             <button onClick={startFirstLesson} className="bg-red-600 hover:bg-red-700 text-white px-8 py-4 rounded font-bold text-xl flex items-center gap-3 transition-transform hover:scale-105">
                ...
                <>
                    <div className="relative aspect-video w-full bg-neutral-900 group overflow-hidden">
                        <div className="absolute top-4 right-4 z-20">
                            <button onClick={(e) => { e.stopPropagation(); handleRegenerateImage(); }} className="bg-black/60 hover:bg-black/80 text-white p-2 rounded-full backdrop-blur transition-all hover:rotate-180" title="Rigenera immagine copertina"><RefreshCw className="w-5 h-5" /></button>
                        </div>
                        {loadingImage ? (
                            ...
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={activeLesson.imageUrl || 'default-bg'} className="absolute inset-0 bg-cover bg-center transition-transform duration-[20s] ease-linear hover:scale-105" style={{ backgroundImage: `url(${activeLesson.imageUrl || localCourse.thumbnailUrl})` }}>
                                <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black/30"></div>
                            </motion.div>
                        )}
                        <div className="absolute bottom-0 left-0 p-8 md:p-12 w-full z-10">
                            <span className="inline-block px-4 py-2 bg-red-600 text-white text-base font-bold rounded mb-4 shadow-lg">LEZIONE {localCourse.modules[activeModuleIndex].lessons.findIndex(l => l.id === activeLesson.id) + 1}</span>
                            <h1 className="text-4xl md:text-6xl font-extrabold mb-4 shadow-black drop-shadow-xl leading-tight">{activeLesson.title}</h1>
                    ...
                    <div className="px-6 md:px-16 py-12">
                        <div className="flex justify-between items-center border-b border-neutral-800 pb-8 mb-10">
                            <div><h2 className="text-4xl font-bold text-white mb-2">Materiale Didattico</h2><p className="text-gray-400 text-xl">Contenuto completo del modulo.</p></div>
                            <div className="flex gap-3"><button onClick={handleDownloadPDF} disabled={!activeLesson.content} className={`bg-neutral-800 hover:bg-neutral-700 p-4 rounded-lg text-gray-300 transition hover:text-white ${!activeLesson.content ? 'opacity-50 cursor-not-allowed' : ''}`} title="Scarica Lezione (PDF/HTML)"><FileText className="w-8 h-8" /></button></div>
                        </div>
                        {loadingContent ? (
                            ...
                            <motion.div key={activeLesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="prose prose-invert prose-2xl max-w-none text-gray-300 leading-loose" dangerouslySetInnerHTML={{ __html: activeLesson.content || "<p class='text-gray-500 italic text-xl'>Seleziona una lezione o clicca 'Genera Corso Completo' per creare i contenuti.</p>" }} />
                        )}
                    </div>
                </>
            )}
        ...
        <div className="lg:w-1/3 bg-[#141414] border-l border-neutral-800 h-full overflow-y-auto custom-scrollbar">
             <div className="p-8">
                <div onClick={() => setActiveLesson(null)} className={`cursor-pointer rounded-xl p-6 mb-6 border transition-all flex items-center gap-4 ${activeLesson === null ? 'bg-neutral-800 border-red-600' : 'bg-neutral-900 border-neutral-800 hover:bg-neutral-800'}`}>
                    <div className={`p-3 rounded-full ${activeLesson === null ? 'bg-red-600 text-white' : 'bg-black text-gray-400'}`}><Home className="w-6 h-6" /></div>
                    <div><h4 className={`font-bold text-xl ${activeLesson === null ? 'text-white' : 'text-gray-300'}`}>Presentazione Corso</h4><p className="text-sm text-gray-500">Home page e introduzione</p></div>
                </div>
                ...
                <div className="bg-gradient-to-br from-neutral-800 to-neutral-900 p-6 rounded-xl mb-10 border border-neutral-700 shadow-2xl">
                    <h3 className="font-bold text-white text-xl mb-4 flex items-center gap-2"><Award className="w-6 h-6 text-yellow-500" />Stato Generazione</h3>
                    ...
                    <p className="text-base text-gray-400 flex justify-between font-medium"><span>{progressPercentage}% completato</span>{progressPercentage === 100 && <span className="text-green-500 font-bold">PRONTO</span>}</p>
                </div>

                <div className="space-y-6 pb-24">
                    {localCourse.modules.map((module, mIdx) => (
                        <div key={mIdx} className="overflow-hidden rounded-xl bg-neutral-900/40 border border-neutral-800/60">
                            <button onClick={() => setActiveModuleIndex(mIdx)} className={`w-full text-left p-6 flex justify-between items-center transition-colors ${activeModuleIndex === mIdx ? 'bg-neutral-800 text-white' : 'hover:bg-neutral-800/50 text-gray-400'}`}>
                                <span className="font-bold text-xl md:text-2xl">{module.title}</span>
                                {activeModuleIndex === mIdx ? <List className="w-6 h-6" /> : <span className="text-base font-mono bg-black/50 px-3 py-1 rounded text-gray-500">{module.lessons.length}</span>}
                            </button>
                            <AnimatePresence>
                                {activeModuleIndex === mIdx && (
                                    <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden">
                                        <div className="border-t border-neutral-800">
                                            {module.lessons.map((lesson, lIdx) => (
                                                <div key={lesson.id} onClick={() => setActiveLesson(lesson)} className={`group flex items-center p-6 cursor-pointer transition-all border-l-4 hover:bg-white/5 ${activeLesson?.id === lesson.id ? 'bg-white/10 border-red-600' : 'border-transparent'}`}>
                                                    ...
                                                    <div className="flex-grow min-w-0">
                                                        <h4 className={`text-xl font-medium truncate mb-2 ${activeLesson?.id === lesson.id ? 'text-white' : 'text-gray-400 group-hover:text-gray-200'}`}>{lesson.title}</h4>
                                                        <span className="text-base text-gray-500 flex items-center gap-2">{lesson.duration} {lesson.imageUrl && <ImageIcon className="w-5 h-5 text-blue-400" />} {currentGeneratingId === lesson.id && <span className="text-red-400 text-sm italic">Generazione...</span>}</span>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
